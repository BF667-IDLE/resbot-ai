...
const sendReply = async (sock, remoteJid, rule, message) => {
  try {
    ...
    if (
      ...
      console.log('Proses play audio untuk:', rule.action.content);
      const songInfo = await searchSong(rule.action.content);
      if (songInfo) {
        console.log('Ditemukan lagu:', songInfo.url);
        const dataYoutubeMP3 = await api.get('/api/downloader/ytplay', { url: songInfo.url });
        console.log('Data YouTube MP3:', dataYoutubeMP3);
        /*
Data YouTube MP3: {
  status: true,
  code: 200,
  data: {
    url: 'https://uploader.autoresbot.com/uploads/1769033663794-audio-1769033709217-73d104e4.m4a',
    size: 3787808,
    format: 'mp3'
  }
}
...
*/
        ...
        if (dataYoutubeMP3.data.size > 94491648) {
          return await sock.sendMessage(
            remoteJid,
            { text: config.error.FILE_TOO_LARGE },
            { quoted: message },
          );
        }
        ...
        await sock.sendMessage(
          remoteJid,
          {
            audio: { url: dataYoutubeMP3.data.url },
            mimetype: 'audio/mp4',
            contextInfo: {
              externalAdReply: {
