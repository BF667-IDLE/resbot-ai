...
import { menuMain, menuOwner } from './menus.js';
...
function displayMenu(remoteJid) {
  const number = normalize(remoteJid);
  let menuText = menuMain(config.name_bot);

  const isOwner = config.owner_number.some((owner) => normalize(owner) === number);

  if (isOwner) {
    menuText += '\n' + menuOwner;
  }

  return Promise.resolve(menuText);
}
